...
class ApiService {
  ...
  static Future<Map<String, dynamic>> analyzeReceipt(Map<String, dynamic> data) async {
    final response = await http.post(Uri.parse('$baseUrl/analyze'), headers: _headers, body: jsonEncode(data));
  ...
  static Future<Map<String, dynamic>> verifyReceipt(String canonicalText, String txSignature) async {
    final response = await http.post(
      ...
      headers: _headers,
      body: jsonEncode({'canonicalText': canonicalText, 'txSignature': txSignature}),
    );
    if (response.statusCode == 200) {
      final decoded = jsonDecode(response.body);
      if (decoded is Map<String, dynamic>) return decoded;
  ...
  static Future<Map<String, dynamic>> getProof(String txSignature) async {
    final response = await http.get(Uri.parse('$baseUrl/proof/$txSignature'), headers: _headers);
    if (response.statusCode == 200) {
      final decoded = jsonDecode(response.body);
      if (decoded is Map<String, dynamic>) return decoded;
  ...
  static Future<Map<String, dynamic>> getProofs() async {
    final response = await http.get(Uri.parse('$baseUrl/proofs'), headers: _headers);
    if (response.statusCode == 200) {
      final decoded = jsonDecode(response.body);
      if (decoded is Map<String, dynamic>) return decoded;
      throw Exception('Proofs list failed: invalid JSON response');
    }
    throw _prettyError('Proofs list failed', response);
  }
